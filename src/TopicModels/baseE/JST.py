...
import random
"""
First joint topic-sentiment model
"""
class JST:
    def __init__(self, id, K, lda_data, config, Words):
        ...
        self.lamda_TS = config['lambda_TS']
    ...
    def find_topicAndEmotion(self, i, word):
        term1 = (self.C_WKE[:,:,word] + self.lamda_TS) / (self.C_KE + self.T_size * self.lamda_TS) #n1/d1
        term2 = (self.C_DE[i] + self.beta_E)/(self.C_DE[i] + self.K*self.config['beta_Z'])  #n3/d2
        term3 = np.multiply((self.C_DKE[i] + self.config['beta_Z']), term2) #n2, d3 is a constant
    ...
    def updateParams(self):
        ...
        temp = self.C_WKE.sum(axis=1) + self.lamda_TS
